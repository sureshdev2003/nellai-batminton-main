...
try {
    ...
    foreach ($slots as $slot) {
        ...
        // For monthly bookings, use the slots table's available_members field
        // For daily bookings, calculate dynamically
        $slotAvailableMembers = (int)$slot['slot_available_members'];
        
        // Calculate final available members
        // For monthly bookings: use slots table value
        // For daily bookings: subtract from max members
        $availableMembers = $slotAvailableMembers - $dailyBookedMembers;
        
        // Ensure available members doesn't go below 0
        $availableMembers = max(0, $availableMembers);
        ...
        $availability[$courtNumber] = [
            ...
            'slot_available_members' => $slotAvailableMembers,
